...
| VCs definidos | 16 |
| VCs automatizados en Iteración 1 | 10 |
| VCs de integración pendientes | 6 |
| VCs pasando al momento de escribir | 10 automatizados; 6 pendientes de entorno GCP o Iteración 2 |
...
| VC-8 | FR-8 progreso | `tests/test_gcsgrep.py::test_scan_reports_progress_every_hundred_objects` | Mensaje de progreso en stderr | ✅ |
...
| VC-16 | NFR-3 scripting | `tests/test_gcsgrep.py` y futura integración | Resultados en stdout, errores en stderr, sin traceback | Parcial |
...
Resultado actual de la verificación local:

```text
10 passed
```

Los VCs marcados como `Pendiente` requieren un bucket, credenciales y/o trabajo
de la Iteración 2. Antes de la entrega final hay que ejecutar los VCs de
integración contra un entorno GCP de prueba o mantenerlos explícitamente como
alcance diferido.
